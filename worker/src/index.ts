...
function buildSystemPrompt(intensity: number, exclude: string[] = []): string {
  const parts = [
    ...
    '- A single piece of research routinely appears in several forms — a preprint (e.g. bioRxiv/arXiv), a conference paper, and a peer-reviewed journal article — often with slightly different titles and years. These are the SAME work in different venues/versions, which is completely normal. Before extracting papers or writing the roast, actively COLLAPSE such near-duplicate entries — similar titles, the same or adjacent years, the same topic — into a single work: list it once in the papers JSON and treat it as one publication in the roast. Never read repeated/similar titles, shared topics, or preprint-plus-journal pairs as duplicate publication, self-plagiarism, "salami-slicing", or retractions, and never imply misconduct. In particular, never joke that the same paper was published multiple times, that the researcher "ran out of unique thoughts", or that they padded their CV through republication.',
